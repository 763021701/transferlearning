    ...
    def update(self, minibatches, opt, sch, epoch):
        ...
        for i in range(nmb):
            for j in range(i + 1, nmb):
                feat = F.normalize(feature_minibatches[i].detach(), dim=1)
                feat_tg = F.normalize(feature_minibatches[j].detach(), dim=1)
        ...
        for i in range(nmb):
            for j in range(i + 1, nmb):
                feat = F.normalize(feature_minibatches[i], dim=1)
                feat_tg = F.normalize(feature_minibatches[j], dim=1)
